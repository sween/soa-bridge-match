...
def update_references(parent: dict):
    ...
    if 'reference' in parent:
        ...
        if parent['reference'].startswith('Organization/'):
        ...
        if parent['reference'].startswith('ResearchStudy/'):
...
def split_bundle(bundle: dict, expected: list[str]) -> dict:
    ...
    for entry in bundle['entry']:
        ...
        if rtype in ("ResearchStudy", "Group", "Organization", "Practitioner", "Medication") or rtype.endswith("Definition"):
            # design elements
            common.append(entry)
        else:
            if rtype == "Patient":
    ...
    if os.path.exists(filename):
        ...
        for entry in data['entry']:
            ...
            if resource['resourceType'] == 'AdverseEvent':
                ...
                patient_ids.append(patch_patient(resource))
        ...
        for patient_id, entries in split_entries.items():
            content = dict(resourceType="Bundle",
                           id=str(uuid.uuid4()),
                           type="transaction",
                           meta=dict(lastUpdated=datetime.now().isoformat()),
                           entry=entries)
